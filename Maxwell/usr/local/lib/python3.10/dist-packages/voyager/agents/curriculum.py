...
class CurriculumAgent:
    ...
    def propose_next_task(self, *, events, chest_observation, max_retries=5):
        if self.progress == 0 and self.mode == "auto":
            task = "Deploy 1 EC2 Instance"
            context = "You can deploy 1 EC2 instance of any size onto AWS"
            return task, context


        # This is for mc inventory management so that inventory is always available
    ...
    def run_qa(self, *, events, chest_observation):
        ...
        questions_new, _, registry_questions, _ = self.run_qa_step1_ask_questions(
            events=events, chest_observation=chest_observation
        )
        questions = []
        answers = []
    ...
    """
    def run_qa_step1_ask_questions(self, *, events, chest_observation, keywords):
