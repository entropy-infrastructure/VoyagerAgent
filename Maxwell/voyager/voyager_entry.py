...
from  voyager.env.VoyagerEnv import VoyagerEnv

from voyager.agents import ActionAgent
...
class Voyager:
    def __init__(
        ...
        """ this sets up the event recording (hence the env dicts -> these are stored on events)"""
    ...
    def reset(self, task, context="", reset_env=True):
        ...
        human_message = self.action_agent.render_human_message(errors="", code="", task=self.task, context=context, critique=""
        )
    ...
    """
    in conversations, append with tuple of sys message, human message, and action agent response from reset() or step()

    parse result = take llm output -> convert it into usable code

    step forward with env given target code to run and available programs (compositional execution portion)

    record events + current task

    take most recent event and add new chests to memory

    use critic agent to check task success

    if reset_placed_if_failed and failed task -> reset all block place events

    presumably, self.context has been set prior to running 'step'

    for event in events only add back to blocks and positions stuff that was present when last save occured

    new skills is retrieved using current context (where is this generated?)

    create new set of self.messages from new skills

    defines run as new rollout (task attempt). if succesful, add parsed code to info returned
    """
    ...
    def step(self):
        ...
        success = False
        dict = self.env.write_prompt(ai_message.content)
        errors = self.env.execute()
        print(errors)
        print("Successfully deployed your server!")
        old_code = dict["pulumi_function_code"] + dict["ansible_function_code"]
        ...
        if success:
            info["pulumi_program_code"] = dict["pulumi_function_code"]
            info["pulumi_program_name"] = dict["pulumi_function_name"]
            info["ansible_program_code"] = dict["ansible_function_code"]
            info["ansible_program_name"] = dict["ansible_function_name"]
        else:
            print(
    ...
    """
    begin with a new sys prompt + skill set of current skills

    reward is set up for an RLHF algo?
    """
    ...
    """
      reset only resets inventory, not skills

      while true
      1. propose task + context (the whole task QA context thing)
      2. self.rollout == self.action_agent.ai_message is parsed and run in the env then evaluated by the critic and info             returned
      3. the info dict is then added to skills upon success
      4. update cirriculum list of completed vs failed tasks
    """

    def get_action_messages(self, code="", task="", context="", critique="",):
        system_message = self.action_agent.render_system_message(skills={"ansible": self.skill_manager.skills["ansible"], "pulumi": self.skill_manager.skills["pulumi"]  } )
        human_message = self.action_agent.render_human_message(
            events=[],
            code=code,
            task=task,
    ...
    def _learn(self):
        ...
        self.messages = self.get_action_messages(task=task, context=context)
    ...
    """
      povided task or list of subgoals

      decomposes task into subgoals

      resets cirriculum

      cirriculum agent progress == num completed tasks

      gets next task context

      does a rollout on that task

      updates skill stuff

      This should automatically take a step towards solving the subtask

      Step may or may not fail, if it fails, it will keep attempting until it fails max retries
    """
